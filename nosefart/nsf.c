...
nsf_t* nsf_load_extended(struct nsf_loader_t* loader)
{
  ...
  if (length <= NSF_HEADER_SIZE) {
    ...
    if (temp_nsf->version > 1) {
        ...
        temp_nsf->length = 0 + temp_nsf->reserved[0] + (temp_nsf->reserved[1] << 8) + (temp_nsf->reserved[2] << 16);
    }
    ...
    while (!loader->read(loader, &nsf_file_ext, sizeof(nsf_file_ext)) && !memcmp(nsf_file_ext.magic, id, 4)) {
        /* Got a NESM extension here. Checks for known extension type :
         * right now, the only extension is "TIME" which give songs length.
         * in frames.
         */
        int size;
        size = 0 + nsf_file_ext.size[0] + (nsf_file_ext.size[1] << 8) + (nsf_file_ext.size[2] << 16) + (nsf_file_ext.size[3] << 24);
        ...
        if (!temp_nsf->song_frames && !memcmp(nsf_file_ext.type, "TIME", 4) && !(size & 3) && (size >= 2 * 4) && (size <= 256 * 4)) {
            ...
            /* Copy time info. */
            for (i = 0; i < tsongs; ++i) {
                temp_nsf->song_frames[i] = 0 | tmp_time[i][0] | (tmp_time[i][1] << 8) | (tmp_time[i][2] << 16) | (tmp_time[i][2] << 24);
            }
